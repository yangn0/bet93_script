...
def lastResult(session: requests.Session, lottery: str, token: str, ssid1: str, random: str):
    headers = {
        ...
        'user-agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36',
        'x-requested-with': 'XMLHttpRequest',
        'cookie': f'983476c949fb={token}; _skin_=red; defaultLT={lottery}; affid=null; token={token}; ssid1={ssid1}',
    }
    params = (
        ('lottery', lottery),
...
def period(session: requests.Session, lottery: str, token: str, ssid1: str, random: str):
    headers = {
        ...
        'user-agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36',
        'x-requested-with': 'XMLHttpRequest',
        'cookie': f'983476c949fb={token}; _skin_=red; defaultLT={lottery}; affid=null; token={token}; ssid1={ssid1}; random={random}',
    }
    params = (
        ('lottery', lottery),
        ('_', f'{round(time.time() * 1000)}'),
...
def bet(session: requests.Session, json_: dict, token: str, ssid1: str, random: str):
    headers = {
        ...
        'referer': f'https://{main_url}/member/index',
        'accept-language': 'zh-CN,zh;q=0.9',
        'cookie': f'983476c949fb={token}; _skin_=red; defaultLT={json_["lottery"]}; affid=null; token={token}; ssid1={ssid1}; random={random}',
    }
    #去掉虚拟投注
...
def draw(session: requests.Session, cardid: str, drawcode: str, drawamount: int, token: str, ssid1: str, random: str):
    ...
    headers = {
        ...
        'accept-language': 'zh-CN,zh;q=0.9',
        'cookie': f'affid=null; 983476c949fb={token}; _skin_=red; defaultLT=SGFT; _bindSecQue=true; affid=null; token={token}; ssid1={ssid1}; random={random}',
    }
    data = {
